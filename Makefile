...
# Quick Commands
up: dev-up ## Start development environment (alias)
...
health: ## Check health of all services
	...
	curl -s http://localhost:3000/health || echo "❌ Frontend not healthy"
	@echo "Backend health:"
	curl -s http://localhost:8000/health || echo "❌ Backend not healthy"
	@echo "Redis health:"
	docker exec cluster-ai-redis redis-cli ping || echo "❌ Redis not healthy"
	@echo "PostgreSQL health:"
	docker exec cluster-ai-postgres pg_isready -U cluster_ai || echo "❌ PostgreSQL not healthy"
