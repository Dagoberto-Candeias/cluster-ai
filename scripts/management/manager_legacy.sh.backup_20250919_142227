...
start_dask() {
    if ! pgrep -f "dask-scheduler" > /dev/null; then
        ...
        local scheduler_log="${PROJECT_ROOT}/logs/dask_scheduler.log"
        # Inicia o scheduler em background e redireciona a saída para um log
        nohup dask-scheduler --host 0.0.0.0 --port 8786 --dashboard-address :8787 > "$scheduler_log" 2>&1 &
        local scheduler_pid=$!

        # Aguarda um momento para o processo iniciar antes de verificar
        sleep 2

        # Verifica se o processo com o PID específico ainda está rodando
        if ps -p $scheduler_pid > /dev/null; then
            success "Dask Scheduler iniciado (PID: $scheduler_pid)."
        else
            error "Falha ao iniciar Dask Scheduler. Verifique o log em: $scheduler_log"
            return 1
        fi
    else
...
run_service_management_menu() {
    while true; do
        ...
        case $choice in
            1) 
                toggle_service "ollama" "service_active '$OLLAMA_SERVICE_NAME'" "start_ollama" "stop_ollama"
               ;;
            2)
                toggle_service "dask" "pgrep -f 'dask-scheduler' > /dev/null" "start_dask" "stop_dask"
               ;;
            3)
                toggle_service "openwebui" "command_exists docker && docker ps --format '{{.Names}}' | grep -q '$OPENWEBUI_CONTAINER_NAME' 2>/dev/null" "start_openwebui" "stop_openwebui"
               ;;
            4) break ;;
            *) warn "Opção inválida." ;;
        esac
