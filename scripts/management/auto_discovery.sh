...
# Script de Descoberta Automática de Workers para Cluster AI
...
# Funções de logging
log_info() {
    echo -e "${BLUE}[INFO]${NC} $1" | tee -a "$DISCOVERY_LOG"
}

log_success() {
    echo -e "${GREEN}[SUCCESS]${NC} $1" | tee -a "$DISCOVERY_LOG"
}

log_warning() {
    echo -e "${YELLOW}[WARNING]${NC} $1" | tee -a "$DISCOVERY_LOG"
}

log_error() {
    echo -e "${RED}[ERROR]${NC} $1" | tee -a "$DISCOVERY_LOG"
}
...
# Descoberta por hostname
discover_by_hostname() {
    local hostname="$1"
    log_info "Tentando resolver hostname: $hostname"

    if ip=$(getent hosts "$hostname" | awk '{print $1}' | head -1); then
        if [[ -n "$ip" ]]; then
            log_success "Hostname $hostname resolvido para $ip"
            echo "$ip"
            return 0
        fi
    fi

    log_warning "Não foi possível resolver hostname: $hostname"
    return 1
}
...
discover_by_arp() {
    ...
    log_info "Escaneando rede com ARP: $network"
    ...
    if command -v arp-scan >/dev/null 2>&1; then
        ...
        log_warning "arp-scan não encontrado. Instale com: sudo apt install arp-scan"
    fi
...
# Limpar workers antigos
cleanup_old_workers() {
    local days="${1:-7}"
    local cutoff_time
    cutoff_time=$(($(date +%s) - (days * 24 * 60 * 60)))

    log_info "Limpando workers não vistos há $days dias..."
    ...
    for worker_id in $(echo "$workers" | jq -r 'keys[]'); do
        ...
        if [[ $last_seen -gt $cutoff_time ]]; then
            cleaned_workers=$(echo "$cleaned_workers" | jq ".\"$worker_id\" = $worker_info")
        else
            log_info "Removendo worker antigo: $worker_id"
            ((removed_count++))
        fi
    done

    echo "$cleaned_workers" > "$WORKERS_FILE"
    log_success "Limpeza concluída: $removed_count workers removidos"
}

# Modo daemon (descoberta automática)
start_daemon() {
    ...
    while true; do
        discover_all
        log_info "Aguardando próximo ciclo de descoberta..."
        sleep "$interval"
    done
}
...
main() {
    echo
    echo "🔍 CLUSTER AI - Sistema de Descoberta Automática"
    echo "=" * 55
    echo
    ...
    case "${1:-help}" in
        ...
        "hostname")
            if [[ -n "$2" ]]; then
                discover_by_hostname "$2"
            else
                log_error "Uso: $0 hostname <nome-do-host>"
            fi
            ;;
        "help"|*)
            ...
            echo "  cleanup [dias]    - Limpar workers antigos (padrão: 7 dias)"
            echo "  daemon [segundos] - Iniciar modo daemon (padrão: 300s)"
            echo "  hostname <nome>   - Resolver hostname para IP"
            echo "  help              - Mostrar esta ajuda"
            echo
            echo "Exemplos:"
            echo "  $0 discover"
            echo "  $0 daemon 60"
            echo "  $0 hostname android-device.local"
            ;;
    esac
}
