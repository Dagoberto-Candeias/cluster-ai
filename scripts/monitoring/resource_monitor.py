...
def get_remote_resources(host: str, user: str = "termux") -> Dict[str, Any]:
    ...
    try:
        ...
        success, stdout, stderr = execute_remote_command(
            host, "top -bn1 | grep 'Cpu(s)' | sed 's/.*, *\\([0-9.]*\\)%* id.*/\\1/' | awk '{print 100 - $1}'", user
        )
        ...
        success, stdout, stderr = execute_remote_command(
            host, "termux-battery-status | grep percentage | cut -d: -f2 | tr -d ' ,' 2>/dev/null || echo 'N/A'", user
        )
        ...
        # Status de rede
        success, stdout, stderr = execute_remote_command(
            host, "ping -c 1 8.8.8.8 >/dev/null 2>&1 && echo 'connected' || echo 'disconnected'", user
        )
...
def get_android_resources(host: str, user: str = "termux") -> Dict[str, Any]:
    ...
    try:
        ...
        success, stdout, stderr = execute_remote_command(
            host, "getprop ro.product.model 2>/dev/null || echo 'Unknown Android Device'", user
        )
        resources['device_model'] = stdout.strip() if success else 'Unknown Android Device'
