...
check_ollama() {
    ...
    if command_exists ollama; then
        ...
        if service_active ollama; then
            ...
            local api_response=$(curl --connect-timeout 30 --max-time 120 --connect-timeout 30 --max-time 120 --connect-timeout 30 --max-time 120 -s -w "%{http_code}" http://localhost:11434/api/tags -o /dev/null 2>/dev/null || echo "000")
        ...
        if confirm_operation "Deseja tentar instalar o Ollama agora?"; then
            if curl --connect-timeout 30 --max-time 120 --connect-timeout 30 --max-time 120 --connect-timeout 30 --max-time 120 -fsSL https://ollama.com/install.sh | sh; then
