...
class AutoWorkerUpdates:
                ...
                if not should_exclude:
                    try:
                        with open(file_path, 'rb') as f:
                            ...
                            checksums.append(f"{file_path.relative_to(self.project_root)}:{file_hash}")
                    except:
    ...
    def ssh_connect(self, hostname: str, ip: str, port: int = 22, user: str = None) -> Optional[paramiko.SSHClient]:
        ...
        try:
            ...
            if ssh_key_file.exists():
                client.connect(ip, port=port, username=user, key_filename=str(ssh_key_file), timeout=10)
    ...
    def sync_project_to_worker(self, ssh_client: paramiko.SSHClient, hostname: str) -> bool:
        ...
        try:
            ...
            rsync_cmd = [
                "rsync", "-avz", "--delete",
                "--rsync-path", f"mkdir -p {worker_project_path} && rsync"
            ] + exclude_args + [
            ...
            stdout, stderr, returncode = self.run_command(rsync_cmd, timeout=300)  # 5 minutes timeout
    ...
    def update_worker_services(self, ssh_client: paramiko.SSHClient, hostname: str) -> bool:
        ...
        try:
            ...
            output, error, returncode = self.execute_remote_command(ssh_client, update_script)
            ...
            output, error, returncode = self.execute_remote_command(ssh_client, start_cmd)
    ...
    def update_all_workers(self) -> Dict[str, bool]:
        ...
        if not self.has_project_changed():
            logger.info("📋 Project unchanged since last update. Skipping worker updates.")
            return {}
        ...
        logger.info(f"📊 Update complete: {successful}/{total} workers updated successfully")

        return results

    def update_worker(self, hostname: str, ip: str, port: int = 22, user: str = None) -> bool:
...
def main():
    ...
    parser.add_argument("action", choices=["update", "check", "force"],
