...
class SmartWorkerDiscovery:
    ...
    def ping_host(self, ip: str) -> bool:
        ...
        stdout, stderr, returncode = self.run_command(["ping", "-c", "1", "-W", "2", ip])
        return returncode == 0
                    ...
                    if ip != self.get_local_ip():
                        ...
                        for port in self.ssh_ports:
                            if self.test_ssh_connection(ip, port):
                                worker_info = {
                                    "hostname": hostname or f"worker-{ip.replace('.', '-')}",
                                    "ip": ip,
                                    "port": port,
                                    "user": os.getenv("USER", "root"),
                                    "status": "active",
                                ...
                                logger.info(f"✅ Found SSH worker: {hostname or 'unknown'} ({ip}:{port})")
                                break
            ...
            if self.ping_host(ip_str):
                ...
                for port in self.ssh_ports:
                    if self.test_ssh_connection(ip_str, port):
                        worker_info = {
                            "hostname": hostname or f"worker-{ip_str.replace('.', '-')}",
                            "ip": ip_str,
                            "port": port,
                            "user": os.getenv("USER", "root"),
                            "status": "active",
                        ...
                        discovered.append(worker_info)
                        logger.info(f"✅ Found SSH worker: {hostname or 'unknown'} ({ip_str}:{port})")
                        break
    ...
    def update_worker_config(self, new_workers: List[Dict]):
        ...
        for worker in new_workers:
            ...
            if hostname not in updated_workers:
                ...
                logger.info(f"🆕 New worker added: {hostname} ({worker['ip']}:{worker['port']})")
            else:
                # Check if IP changed
                existing_ip = updated_workers[hostname]["ip"]
                new_ip = worker["ip"]
                if existing_ip != new_ip:
                    logger.info(f"🔄 Worker {hostname} IP changed: {existing_ip} -> {new_ip}")
        ...
        with open(self.config_file, 'w') as f:
            ...
            for hostname, config in updated_workers.items():
                f.write(f"{hostname} {config['alias']} {config['ip']} {config['user']} {config['port']} {config['status']}\n")

        logger.info(f"✅ Worker configuration updated: {len(updated_workers)} workers configured")
