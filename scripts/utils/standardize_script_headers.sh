...
extract_script_info() {
    ...
    local title=$(grep -E "^#\s*[A-Z].*" "$script_path" | head -1 | sed 's/^#\s*//')
    if [ -z "$title" ]; then
        title="$script_name"
    fi

    # Extrair descrição se existir
    local description=$(grep -A 5 "^#\s*$title" "$script_path" | grep -E "^#\s*[A-Z].*" | head -2 | tail -1 | sed 's/^#\s*//')
...
generate_standard_header() {
    local script_path="$1"
    local info=$(extract_script_info "$script_path")
    local title=$(echo "$info" | cut -d'|' -f1)
    local description=$(echo "$info" | cut -d'|' -f2)
    local script_name=$(echo "$info" | cut -d'|' -f3)
...
standardize_script() {
    local script_path="$1"
    local backup_path="${script_path}.backup_$(date +%Y%m%d_%H%M%S)"
    ...
    local new_header=$(generate_standard_header "$script_path")
...
main() {
    ...
    local scripts=$(find_all_scripts)
