...
install_vscode() {
    ...
    case $OS in
        ubuntu|debian)
            ...
            wget --timeout=30 --tries=2 --timeout=30 --tries=2 --timeout=30 --tries=2 --timeout=30 --tries=2 --timeout=30 --tries=2 --timeout=30 --tries=2 -qO- https://packages.microsoft.com/keys/microsoft.asc | gpg --dearmor > packages.microsoft.gpg
            ...
            sudo apt update
            sudo apt install -y --no-install-recommends --no-install-recommends --no-install-recommends --no-install-recommends --no-install-recommends --no-install-recommends code
        ...
        centos|rhel|fedora)
            ...
            if command_exists dnf; then
                sudo dnf install -y --setopt=install_weak_deps=False --setopt=install_weak_deps=False --setopt=install_weak_deps=False --setopt=install_weak_deps=False --setopt=install_weak_deps=False --setopt=install_weak_deps=False code
            else
...
install_fira_font() {
    ...
    case $OS in
        ubuntu|debian)
            sudo apt install -y --no-install-recommends --no-install-recommends --no-install-recommends --no-install-recommends --no-install-recommends --no-install-recommends fonts-firacode
            ;;
        manjaro|arch)
            sudo pacman -S --noconfirm ttf-fira-code
            ;;
        centos|rhel|fedora)
            sudo dnf install -y --setopt=install_weak_deps=False --setopt=install_weak_deps=False --setopt=install_weak_deps=False --setopt=install_weak_deps=False --setopt=install_weak_deps=False --setopt=install_weak_deps=False fira-code-fonts || sudo yum install -y fira-code-fonts
            ;;
    esac
