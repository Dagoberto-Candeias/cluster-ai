#!/bin/bash
# Script para inicialização automática dos workers Dask com SSD externo e swap configurado

set -euo pipefail

# Add /usr/sbin to PATH for swapon command
...
setup_directories() {
    ...
    # Removendo chmod para evitar erro de permissão em SSD externo montado
    # chmod 755 "$DASK_SPILL_DIR"
    # chmod 755 "$LOG_DIR"
    # chmod 700 "${EXTERNAL_SSD}/swap"

    log "✅ Diretórios preparados"
    log "   Spill dir: $DASK_SPILL_DIR"
    log "   Log dir: $LOG_DIR"
    log "   Swap dir: ${EXTERNAL_SSD}/swap"
}

# Configurar swap no SSD externo
setup_swap() {
    section "Configurando Swap no SSD Externo"

    local swap_file="${EXTERNAL_SSD}/swap/swapfile"

    if [ ! -f "$swap_file" ]; then
        log "Criando swap file de 16GB no SSD externo..."
        fallocate -l 16G "$swap_file" 2>/dev/null || dd if=/dev/zero of="$swap_file" bs=1M count=16384
        chmod 600 "$swap_file"
        mkswap "$swap_file"
        if command -v swapon >/dev/null 2>&1; then
            /usr/sbin/swapon "$swap_file"
            log "✅ Swap de 16GB criado e ativado no SSD externo"
        else
            log "⚠️ Comando swapon não encontrado, swap não ativado automaticamente"
        fi
    else
        if command -v swapon >/dev/null 2>&1; then
            if ! /usr/sbin/swapon --show | grep -q "$swap_file"; then
                /usr/sbin/swapon "$swap_file"
                log "✅ Swap existente ativado"
            else
                log "Swap já está ativo"
            fi
        else
            log "⚠️ Comando swapon não encontrado, swap não ativado automaticamente"
        fi
    ...
    # Adicionar ao fstab para ativação automática
    if ! grep -q "$swap_file" /etc/fstab; then
        echo "$swap_file none swap sw 0 0" | sudo tee -a /etc/fstab >/dev/null
        log "✅ Swap adicionado ao fstab para ativação automática"
    fi

    # Otimizar parâmetros do sistema para swap
    if [ -w /etc/sysctl.conf ]; then
        local sysctl_config="vm.swappiness=10
...
vm.dirty_background_ratio=5"
        ...
        if ! grep -q "vm.swappiness" /etc/sysctl.conf; then
            ...
            log "✅ Configurações de swap otimizadas no sysctl"
        fi
    fi
}
