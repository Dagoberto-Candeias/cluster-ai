...
class TestModelInstallation:
    ...
    def test_ollama_pull_success(self, mock_run):
        ...
        mock_run.return_value = MagicMock(returncode=0, stdout='Successfully pulled model')

        result = subprocess.run(['ollama', 'pull', 'llama3:8b'], capture_output=True, text=True)
    ...
    def test_ollama_pull_failure(self, mock_run):
        ...
        result = subprocess.run(['ollama', 'pull', 'invalid-model'], capture_output=True, text=True)
...
Details: llama3:8b"""
        ...
        result = subprocess.run(['ollama', 'show', 'llama3:8b'], capture_output=True, text=True)
    ...
    def test_ollama_remove(self, mock_run):
        ...
        mock_run.return_value = MagicMock(returncode=0, stdout='Model removed successfully')

        result = subprocess.run(['ollama', 'rm', 'llama3:8b'], capture_output=True, text=True)
...
class TestModelAPI:
    ...
    def test_ollama_api_chat(self, mock_post):
        ...
        response = requests.post('http://localhost:11434/api/chat',
                               json={'model': 'llama3:8b', 'messages': [{'role': 'user', 'content': 'Olá'}]})
...
class TestModelCategories:
    ...
    def test_llm_category_models(self):
        ...
        llm_models = [
            ...
            'codellama:7b', 'deepseek-coder:6.7b', 'qwen:7b', 'phi:2.7b'
        ]
...
class TestModelPerformance:
    ...
    def test_model_inference_time(self, mock_run, mock_time):
        ...
        result = subprocess.run(['ollama', 'run', 'llama3:8b', 'Hello'], capture_output=True, text=True)
        end_time = mock_time()
...
class TestModelCaching:
    ...
    def test_cache_size_calculation(self, mock_getsize, mock_listdir):
        ...
        mock_getsize.side_effect = [1024 * 1024 * 500, 1024 * 1024 * 300]  # 500MB, 300MB

        total_size = sum(os.path.getsize(os.path.join('cache', f)) for f in os.listdir('cache'))
