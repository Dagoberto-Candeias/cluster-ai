...
class TestScriptSyntax:
    ...
    def test_verify_syntax_script_executable(self):
        ...
        script_path = "scripts/verify_syntax.sh"
        assert os.access(script_path, os.X_OK), "Script verify_syntax.sh não é executável"
...
class TestScriptFunctionality:
    ...
    def test_worker_manager_list_command(self, mock_run):
        ...
        mock_run.return_value = MagicMock(returncode=0, stdout="Workers: worker-001, worker-002")

        result = subprocess.run(['bash', 'scripts/management/worker_manager.sh', 'list'],
    ...
    def test_health_check_script(self, mock_run):
        ...
        result = subprocess.run(['bash', 'scripts/health_check.sh'],
                              capture_output=True, text=True)
    ...
    def test_download_models_script(self, mock_run):
        ...
        mock_run.return_value = MagicMock(returncode=0, stdout="Model llama3:8b downloaded")

        result = subprocess.run(['bash', 'scripts/download_models.sh', '--category', 'llm'],
                              capture_output=True, text=True)
            ...
            for root, dirs, files in os.walk(script_dir):
                for file in files:
                    if file.endswith('.sh'):
                        ...
                        assert not (stat_info.st_mode & 0o6000), f"Script {filepath} tem SUID/SGID"
        ...
        if os.path.exists(script_dir):
            for root, dirs, files in os.walk(script_dir):
                for file in files:
                    if file.endswith('.sh'):
                        filepath = os.path.join(root, file)
                        assert os.access(filepath, os.R_OK), f"Script {filepath} não é legível"
                ...
                for file in files:
                    if file.endswith('.sh'):
                        ...
                        with open(filepath, 'r', encoding='utf-8', errors='ignore') as f:
                            ...
                            assert first_line == "#!/bin/bash", f"Script {filepath} não usa #!/bin/bash"
                    ...
                    if file.endswith('.sh'):
                        ...
                        with open(filepath, 'r', encoding='utf-8', errors='ignore') as f:
                            ...
                            if not has_error_handling:
                                pytest.skip(f"Script {filepath} pode não ter tratamento de erro adequado")
...
class TestInstallationScripts:
    ...
    def test_install_unified_script(self, mock_run):
        ...
        result = subprocess.run(['bash', 'install_unified.sh', '--help'],
                              capture_output=True, text=True)
    ...
    def test_termux_worker_setup_exists(self, mock_run):
        ...
        assert os.path.exists(script_path), "Script termux_worker_setup.sh não encontrado"

        # Verificar se é executável
        assert os.access(script_path, os.X_OK), "Script termux_worker_setup.sh não é executável"
...
class TestMonitoringScripts:
    ...
    def test_advanced_dashboard_script(self, mock_run):
        ...
        mock_run.return_value = MagicMock(returncode=0, stdout="Dashboard started on port 3001")

        result = subprocess.run(['bash', 'scripts/monitoring/advanced_dashboard.sh', '--help'],
                              capture_output=True, text=True)
    ...
    def test_log_analyzer_script(self, mock_run):
        ...
        result = subprocess.run(['bash', 'scripts/monitoring/log_analyzer.sh'],
                              capture_output=True, text=True)
