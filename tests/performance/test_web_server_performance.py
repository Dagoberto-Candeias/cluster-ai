...
class TestWebServerPerformance:
    ...
    def test_web_server_response_time(self):
        ...
        assert avg_response_time < 0.1, f"Average response too slow: {avg_response_time:.3f}s"
        # Max response should be under 500ms
        assert max_response_time < 0.5, f"Max response too slow: {max_response_time:.3f}s"
    ...
    def test_concurrent_requests(self):
        ...
        assert avg_response_time < 1.0, f"Average response under load too slow: {avg_response_time:.3f}s"
        assert max_response_time < 3.0, f"Max response under load too slow: {max_response_time:.3f}s"
    ...
    def test_memory_usage_during_load(self):
        ...
        threads = []
        for i in range(50):
            t = threading.Thread(target=lambda: requests.get("http://localhost:8080/", timeout=5))
            threads.append(t)
            t.start()
    ...
    def test_port_conflict_handling(self):
        ...
        if result.returncode == 0:
            ...
            assert "já está rodando" in result.stderr or "Address already in use" in result.stderr
    ...
    def test_error_handling_performance(self):
        ...
        assert error_response_time < 0.5, f"Error response too slow: {error_response_time:.3f}s"
