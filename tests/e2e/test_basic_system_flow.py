    ...
    def test_script_permissions(self):
        ...
        for script_path in script_files:
            ...
            if full_path.exists():
                ...
                assert os.access(full_path, os.X_OK), f"{script_path} should be executable"

                # Should have reasonable permissions (not world-writable)
                stat_info = os.stat(full_path)
                permissions = oct(stat_info.st_mode)[-3:]
                assert permissions[-1] not in ["2", "3", "6", "7"], f"{script_path} should not be world-writable"
...
debug=true
    ...
    def test_environment_specific_config(self):
        ...
        assert current_env in ["development", "testing", "staging", "production", "dev", "test", "prod"]
