...
class TestModelInstaller:
    ...
    def setup_method(self):
        ...
        self.script_path = self.test_dir / "scripts" / "management" / "install_models.sh"
    ...
    def test_script_executable(self):
        ...
        assert os.access(self.script_path, os.X_OK), "Script deve ter permissões de execução"
    ...
    def test_color_variables_defined(self):
        ...
        with open(self.script_path, 'r') as f:
            ...
            # Verifica variáveis de cor do common.sh
            assert 'RED=' in content or 'source.*common.sh' in content
            assert 'GREEN=' in content or 'source.*common.sh' in content
            assert 'YELLOW=' in content or 'source.*common.sh' in content
    ...
    def test_ollama_command_available(self, mock_subprocess):
        ...
        mock_subprocess.return_value = MagicMock(returncode=0, stdout="ollama version 0.1.0")
    ...
    def test_model_count_reasonable(self):
        ...
        with open(self.script_path, 'r') as f:
            ...
            # Conta ocorrências de modelos (formato nome:versão)
            import re
            model_pattern = r'\b\w+:\d+\.\d+\w*\b'
            models = re.findall(model_pattern, content)

            # Deve ter pelo menos alguns modelos
            assert len(models) >= 10, f"Encontrados apenas {len(models)} modelos, esperado pelo menos 10"

            # Não deve ter modelos duplicados consecutivos
            unique_models = set(models)
            assert len(unique_models) == len(models), "Encontrados modelos duplicados"
    ...
    def test_category_descriptions_exist(self):
        ...
        with open(self.script_path, 'r') as f:
            ...
            assert 'Especialista em programação' in content or 'otimizado para' in content
    ...
    def test_script_uses_getopts_or_shift(self):
        ...
        with open(self.script_path, 'r') as f:
            ...
            has_args_processing = (
                'getopts' in content or
                'shift' in content or
                '$1' in content or
                '--help' in content
            )
            assert has_args_processing, "Script deve processar argumentos de linha de comando"
    ...
    def test_logging_available(self):
        ...
        with open(self.script_path, 'r') as f:
            ...
            has_logging = any(func in content for func in ['log(', 'success(', 'warn(', 'error('])
...
class TestModelCategories:
    ...
    def test_conversation_models_exist(self):
        ...
        script_path = Path(__file__).parent.parent.parent / "scripts" / "management" / "install_models.sh"
        ...
        with open(script_path, 'r') as f:
            ...
            assert found_models >= 2, f"Encontrados apenas {found_models} modelos de conversação"
    ...
    def test_programming_models_exist(self):
        ...
        with open(script_path, 'r') as f:
            ...
            assert found_models >= 1, f"Encontrados apenas {found_models} modelos de programação"
...
class TestModelInstallerIntegration:
    ...
    def test_common_library_available(self):
        ...
        common_path = Path(__file__).parent.parent.parent / "scripts" / "lib" / "common.sh"
        assert common_path.exists(), "Biblioteca common.sh deve existir"

        # Testa se common.sh pode ser sourceado
        result = subprocess.run(
            ['bash', '-n', str(common_path)],
            capture_output=True,
            text=True
        )

        assert result.returncode == 0, f"common.sh tem erros de sintaxe: {result.stderr}"
