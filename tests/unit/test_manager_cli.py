...
@pytest.mark.parametrize("command,args,expected_call", [
    ...
    ("health", [], "bash scripts/health_check.sh"),
    ("backup", [], "bash scripts/maintenance/backup_manager.sh"),
    ("restore", [], "bash scripts/maintenance/restore_manager.sh"),
    ("scale", ["3", "--backend", "docker"], "docker compose up -d --scale dask-worker=3"),
    ("scale", ["5", "--backend", "k8s"], "kubectl scale deployment dask-worker --replicas=5"),
    ("scale", ["2", "--backend", "helm"], "helm upgrade cluster-ai ./cluster-ai --set worker.replicas=2"),
])
