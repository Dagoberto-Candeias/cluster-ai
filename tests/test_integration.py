...
class TestSystemIntegration:
    ...
    def test_worker_discovery_integration(self):
        ...
        with patch('subprocess.run') as mock_run:
            mock_run.return_value = MagicMock(returncode=0, stdout="Workers discovered: 2")

            result = subprocess.run(['bash', 'scripts/management/auto_discovery.sh'],
    ...
    def test_model_management_integration(self):
        ...
        with patch('subprocess.run') as mock_run:
            mock_run.return_value = MagicMock(returncode=0, stdout="Model llama3:8b ready")
    ...
    def test_workers_endpoint_integration(self, api_base_url):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                # Testar GET /workers
                response = requests.get(f"{api_base_url}/workers", headers=headers, timeout=5)
                assert response.status_code in [200, 401, 403]  # 401/403 se token inválido

                # Testar GET /cluster/status
                response = requests.get(f"{api_base_url}/cluster/status", headers=headers, timeout=5)
        ...
        for script in security_scripts:
            if os.path.exists(script):
                with patch('subprocess.run') as mock_run:
                    ...
                    result = subprocess.run(['bash', script, '--check'], capture_output=True)
                    assert result.returncode == 0
...
class TestPerformanceIntegration:
        ...
        for script in perf_scripts:
            if os.path.exists(script):
                with patch('subprocess.run') as mock_run:
                    ...
                    result = subprocess.run(['bash', script, '--status'], capture_output=True)
                    assert result.returncode == 0
    ...
    def test_load_test_simulation(self):
        ...
        with patch('subprocess.run') as mock_run:
            ...
            for i in range(5):
                result = subprocess.run(['bash', 'scripts/health_check.sh'], capture_output=True)
                assert result.returncode == 0
                time.sleep(0.1)
