...
from main_fixed import app, get_current_user, create_access_token, init_default_user, User
...
class TestAuthEndpoints:
    ...
    def test_protected_endpoint_with_valid_token(self, mock_user):
        ...
        mock_user.return_value = User(username="admin", email="admin@example.com", full_name="Administrator", disabled=False)
...
class TestLogsEndpoints:
    ...
    def test_get_logs_with_filter(self, mock_user, mock_listdir, mock_exists):
        ...
        with patch('builtins.open', mock_open(read_data="[2023-01-01 10:00:00] [INFO] [SYSTEM] Test log\n")):
            token = create_access_token(data={"sub": "admin"})
            headers = {"Authorization": f"Bearer {token}"}
...
class TestClusterStatusEndpoints:
    ...
    @patch('main_fixed.get_current_user', new_callable=AsyncMock)
    def test_get_cluster_status(self, mock_user, mock_alerts, mock_workers, mock_check_service,
        ...
        mock_workers.return_value = [{"status": "active", "cpu_usage": 30.0, "memory_usage": 40.0}]
