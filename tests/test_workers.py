...
class TestWorkerManager:
    ...
    def test_list_workers_with_yq(self, temp_config):
        ...
        with patch('subprocess.run') as mock_run, \
             patch('shutil.which', return_value='/usr/bin/yq'):

            mock_run.return_value = MagicMock(stdout='worker-01\nworker-02', returncode=0)

            # Simular execução do script
            result = subprocess.run([
                'bash', 'scripts/management/worker_manager.sh', 'list'
            ], capture_output=True, text=True, cwd=os.path.dirname(temp_config))
    ...
    def test_start_worker_success(self, mock_run):
        ...
        result = subprocess.run([
            'bash', 'scripts/management/worker_manager.sh', 'start-worker', 'worker-01'
    ...
    def test_stop_worker_success(self, mock_run):
        ...
        result = subprocess.run([
            'bash', 'scripts/management/worker_manager.sh', 'stop-worker', 'worker-01'
        ], capture_output=True, text=True)
    ...
    def test_add_worker_interactive_simulation(self):
        ...
        with patch('subprocess.run') as mock_run, \
             patch('builtins.input', side_effect=['worker-03', '192.168.1.102', 'testuser', '22', 'y']):
            ...
            ], capture_output=True, text=True, input='worker-03\n192.168.1.102\ntestuser\n22\ny\n')
...
class TestWorkerConnectivity:
    ...
    def test_ssh_connection_success(self, mock_run):
        ...
        mock_run.return_value = MagicMock(returncode=0, stdout='SSH connection successful')

        result = subprocess.run(['ssh', '-T', 'user@host'], capture_output=True, text=True)
