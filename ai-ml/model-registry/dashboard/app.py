...
def index():
    ...
    try:
        ...
        stats = {
            ...
            "last_updated": (
                max(m.get("created_at", "") for m in models) if models else None
            ),
        }
    ...
    if request.method == "POST":
        try:
            ...
            if not all([model_path, name, framework]):
                flash(
                    "Campos obrigatórios: caminho do modelo, nome e framework", "error"
                )
...
def api_register():
    ...
    try:
        ...
        return jsonify(
            {"success": True, "message": f'Modelo "{data["name"]}" registrado'}
        )
...
def welcome():
    """Endpoint que retorna uma mensagem de boas-vindas."""
